    ...
    for (BTRGridLayoutSection *section in _data.sections) {
        if (CGRectIntersectsRect(section.frame, rect)) {
			...
			if (!CGRectIsEmpty(normalizedHeaderFrame) && CGRectIntersectsRect(normalizedHeaderFrame, rect)) {
				BTRCollectionViewLayoutAttributes *layoutAttributes = [[[self class] layoutAttributesClass] layoutAttributesForSupplementaryViewOfKind:BTRCollectionElementKindSectionHeader withIndexPath:[NSIndexPath btr_indexPathForItem:0 inSection:sectionIndex]];
			...
			for (BTRGridLayoutRow *row in section.rows) {
                ...
                if (CGRectIntersectsRect(normalizedRowFrame, rect)) {
                    ...
                    for (NSInteger itemIndex = 0; itemIndex < row.itemCount; itemIndex++) {
                        ...
                        layoutAttributes = [[[self class] layoutAttributesClass] layoutAttributesForCellWithIndexPath:[NSIndexPath btr_indexPathForItem:sectionItemIndex inSection:sectionIndex]];
			...
			if (!CGRectIsEmpty(normalizedFooterFrame) && CGRectIntersectsRect(normalizedFooterFrame, rect)) {
				BTRCollectionViewLayoutAttributes *layoutAttributes = [[[self class] layoutAttributesClass] layoutAttributesForSupplementaryViewOfKind:BTRCollectionElementKindSectionFooter withIndexPath:[NSIndexPath btr_indexPathForItem:0 inSection:sectionIndex]];
...
- (BTRCollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BTRCollectionViewLayoutAttributes *layoutAttributes = [[[self class] layoutAttributesClass] layoutAttributesForCellWithIndexPath:indexPath];
...
- (BTRCollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    BTRCollectionViewLayoutAttributes *layoutAttributes = nil;
    ...
    if (sectionIndex < _data.sections.count) {
        ...
        if (!CGRectIsEmpty(normilazedFrame)) {
            ...
            layoutAttributes = [[[self class] layoutAttributesClass] layoutAttributesForSupplementaryViewOfKind:kind withIndexPath:[NSIndexPath btr_indexPathForItem:0 inSection:sectionIndex]];
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    ...
    if ((self.collectionView.bounds.size.width != newBounds.size.width && self.scrollDirection == BTRCollectionViewScrollDirectionVertical) || (self.collectionView.bounds.size.height != newBounds.size.height && self.scrollDirection == BTRCollectionViewScrollDirectionHorizontal)) {
        return YES;
    }
    return NO;
}
